...
-- function owned by ordaro_owner. Each is keyed by an id or an unguessable hash and returns only
-- the columns its caller needs, so the privileged surface stays small and readable.
--
-- The app role owns nothing and has no BYPASSRLS (step 1); ordaro_owner, the table owner, is not
...
create function ordaro.auth_membership(p_membership_id uuid)
    returns table (id uuid, organization_id uuid, account_id uuid, role varchar, status varchar, location_id uuid)
    language sql stable security definer set search_path = ordaro, pg_temp
as $$
    select m.id, m.organization_id, m.account_id, m.role, m.status, m.location_id
    from ordaro.membership m where m.id = p_membership_id
$$;

-- One account's active membership in one organization: the switch.
create function ordaro.auth_membership_active(p_account_id uuid, p_organization_id uuid)
    returns table (id uuid, organization_id uuid, account_id uuid, role varchar, status varchar, location_id uuid)
    language sql stable security definer set search_path = ordaro, pg_temp
as $$
    select m.id, m.organization_id, m.account_id, m.role, m.status, m.location_id
    from ordaro.membership m
...
create function ordaro.auth_memberships_for_account(p_account_id uuid)
...
as $$
    ...
    from ordaro.membership m
    join ordaro.organization o on o.id = m.organization_id
...
create function ordaro.auth_membership_by_invite(p_code_hash varchar)
...
as $$
    ...
    from ordaro.membership m where m.invite_code_hash = p_code_hash
$$;

-- A register by id: the per-request check of a REGISTER token.
create function ordaro.auth_register_device(p_device_id uuid)
...
as $$
    ...
    from ordaro.register_device d where d.id = p_device_id
$$;
...
create function ordaro.auth_register_device_by_credential(p_credential_hash varchar, p_lock boolean)
    returns table (id uuid, organization_id uuid, location_id uuid, status varchar, expires_at timestamptz,
                   failed_pin_count integer, locked_until timestamptz)
    language plpgsql volatile security definer set search_path = ordaro, pg_temp
as $$
begin
    if p_lock then
        return query
            select d.id, d.organization_id, d.location_id, d.status, d.expires_at, d.failed_pin_count, d.locked_until
            from ordaro.register_device d where d.credential_hash = p_credential_hash for update;
    else
        return query
            select d.id, d.organization_id, d.location_id, d.status, d.expires_at, d.failed_pin_count, d.locked_until
            from ordaro.register_device d where d.credential_hash = p_credential_hash;
    end if;
end;
$$;

revoke all on function ordaro.auth_membership(uuid) from public;
revoke all on function ordaro.auth_membership_active(uuid, uuid) from public;
revoke all on function ordaro.auth_memberships_for_account(uuid) from public;
revoke all on function ordaro.auth_membership_by_invite(varchar) from public;
revoke all on function ordaro.auth_register_device(uuid) from public;
revoke all on function ordaro.auth_register_device_by_credential(varchar, boolean) from public;

grant execute on function ordaro.auth_membership(uuid) to ordaro_app;
grant execute on function ordaro.auth_membership_active(uuid, uuid) to ordaro_app;
grant execute on function ordaro.auth_memberships_for_account(uuid) to ordaro_app;
grant execute on function ordaro.auth_membership_by_invite(varchar) to ordaro_app;
grant execute on function ordaro.auth_register_device(uuid) to ordaro_app;
grant execute on function ordaro.auth_register_device_by_credential(varchar, boolean) to ordaro_app;
...
begin
    for t in
        ...
        where c.table_schema = 'ordaro' and c.column_name = 'organization_id'
        order by c.table_name
    loop
        execute format('alter table ordaro.%I enable row level security', t);
        execute format($p$
            create policy tenant_isolation on ordaro.%I
...
create policy membership_own_account on ordaro.membership
    for select
