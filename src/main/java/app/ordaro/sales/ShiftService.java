 ...
 * register is in the drawer. Complete as of step 5d.
 */
@Service
public class ShiftService {
    ...
    public record Drawer(BigDecimal openingFloat, BigDecimal cashSales, BigDecimal cashRepayments,
            BigDecimal cashRefunds, BigDecimal cashExpenses, BigDecimal cashSupplierPayments,
    ...
    private Drawer drawerOf(CashierShift shift) {
        ...
        BigDecimal expenses = shifts.cashSpent(shift.getId());
        BigDecimal suppliers = shifts.cashPaidToSuppliers(shift.getId());
        return new Drawer(shift.getOpeningFloat(), sales, repayments, refunds, expenses, suppliers,
                shift.getOpeningFloat().add(sales).add(repayments).subtract(refunds).subtract(expenses)
                        .subtract(suppliers));
    }
