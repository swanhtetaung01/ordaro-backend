...
public class TokenService {
    ...
    public IssuedTokens forMembership(UUID accountId, MembershipSnapshot membership, UUID familyId,
        ...
        return issue(claims, now, accountId, membership.id(), RefreshTokenKind.USER, familyId, deviceLabel);
    }
    ...
    public IssuedTokens forPicker(UUID accountId, UUID familyId, String deviceLabel) {
        Instant now = clock.instant();
        return issue(baseClaims(now, RefreshTokenKind.PICKER, accountId), now, accountId, null,
    ...
    private IssuedTokens issue(JwtClaimsSet.Builder claims, Instant now, UUID accountId, UUID membershipId,
            RefreshTokenKind kind, UUID familyId, String deviceLabel) {
        ...
        refreshTokens.save(new RefreshToken(accountId, membershipId, Secrets.sha256Hex(refreshToken),
